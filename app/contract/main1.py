...
def escrow_task_payment(task: TaskInDB):
    call_result: CallResult[TransferResult] = token_canister.transfer(ic.id(), task.max_price).call()
...
def pay_task_executor(task: TaskInDB):
    call_result: CallResult[TransferResult] = token_canister.transfer(task.executed_by_id, task.max_price).call()
...
def refund_task_payment(task: TaskInDB):
    call_result: CallResult[TransferResult] = token_canister.transfer(task.requested_by_id, task.max_price).call()
...
def create_task(create_task_data: TaskCreate, current_user: CurrentUser) -> UUID4:
    ...
    task = TaskInDB.create(create_task_data)
    escrow_result = escrow_task_payment(task)
...
def complete_task(task_id: UUID4, current_user: CurrentUser):
    ...
    payment_result = pay_task_executor(task)
